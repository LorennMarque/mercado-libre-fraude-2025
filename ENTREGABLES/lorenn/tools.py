...
def optimizar_threshold_costo_cv(model, X, y, cv, costo_fp=5.0, costo_fn=100.0, 
    ...
    # Verificar si el modelo tiene early_stopping_rounds configurado (XGBoost, LightGBM, etc.)
    # Si es así, lo deshabilitamos temporalmente para CV ya que no tenemos eval_set
    original_early_stopping = None
    if hasattr(model, 'early_stopping_rounds') and model.early_stopping_rounds is not None:
        original_early_stopping = model.early_stopping_rounds
    
    for fold_idx, (train_idx, test_idx) in enumerate(cv.split(X, y)):
        # Entrenar modelo en el fold de entrenamiento
        model_fold = clone(model)
        # Deshabilitar early stopping para CV si estaba habilitado
        if original_early_stopping is not None:
            model_fold.set_params(early_stopping_rounds=None)
